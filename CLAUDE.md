...
- `ipc.rs` — unix-socket IPC mirroring alacritty's `polling/ipc.rs`: socket at `$XDG_RUNTIME_DIR/alacritree/alacritree-<pid>.sock`, advertised via `ALACRITREE_SOCKET`, one newline-delimited JSON request per connection with an `{"ok"}/{"error"}` reply. Requests that touch app state are forwarded to the UI thread as `AppCall`s (drained in `update`, woken by `request_repaint`); slow ones (git status, worktree creation) run on the connection thread. Disabled via `[general] ipc_socket = false`.
- `mcp.rs` — `alacritree mcp`: a hand-rolled stdio MCP server (newline-delimited JSON-RPC, tools only) whose tool names/arguments map 1:1 onto `ipc::IpcRequest` serde tags. Deliberately SDK-free to keep the crate synchronous; unix-only like the socket it bridges to.
