...
- Schema defaults: every config key whose omission resolves to a fixed value must publish that value in the JSON Schema `default` field with its actual JSON type. Keep the default in the relevant `Raw*` type's `Default` implementation under `#[serde(default)]`, so runtime resolution and schema generation share it; collection elements must support serialization for their defaults to appear. Preserve meaningful omission for required, inherited, or runtime-derived values, and explain exceptions in field documentation and `alacritree/tests/schema-defaults-allowlist.txt`. After regeneration, require the `config_schema` and `schema_defaults` tests to pass; review each allowlist change against omission behavior.
