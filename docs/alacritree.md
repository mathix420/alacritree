...
attach           = "agent"  # what opening a row attaches to. "agent" opens the
                            ...
                            # mouse to herdr's own client, so a selection joins
                            # soft-wrapped rows and copy mode works. Honoured
                            # per side: the native side of a Windows host always
                            # gets "session", because herdr implements no direct
                            # attach there
