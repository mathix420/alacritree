...
The [configuration reference](config-reference.md) lists every key with its type, default and what it does. It is generated from the same doc comments as the JSON Schema below, so the two cannot disagree.

`[ui.paste]` lets Paste handle a clipboard with no text. Text always wins: a
clipboard carrying both text and an image pastes the text. Only the regular
clipboard is checked for files and images. The X11 PRIMARY selection is text,
so middle-click paste is unchanged. `files` and `image` both `false` restores the original behavior exactly, where a paste with no text does nothing.
