...
pub fn emit(config: &Config, files: &[ConfigFile], settings: bool) {
    log::info!("alacritree {} on {}", env!("CARGO_PKG_VERSION"), std::env::consts::OS);
    for file in files {
        log::info!("config {}", describe(file));
    }
    if settings {
...
fn describe(file: &ConfigFile) -> String {
    match (&file.path, &file.error) {
        ...
        (None, _) => format!("{}.toml: not found on the search path", file.stem),
    }
}
...
mod tests {
    ...
    fn a_missing_file_says_so_rather_than_printing_an_empty_path() {
        ...
        assert_eq!(describe(&file), "alacritty.toml: not found on the search path");
    }
}
